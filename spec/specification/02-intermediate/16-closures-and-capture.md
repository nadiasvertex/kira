...
- The `move` keyword parses (`parser.cpp` sets `lambda_expr::is_move` on `kw_move`) and survives AST cloning. Combined with an explicit capture list, `move` now has real effect: `parse_lambda_expr` promotes every bare `name` entry in the list to `ast::capture_mode::by_move` (`&name`/`&mut name` entries are untouched, since they already state their own mode); `src/semantic/move_check.cpp`'s `check_lambda` consumes each `by_move` entry against the enclosing scope at the point the lambda is created, so reusing the original binding afterward is a use-after-move error, the same as any other move. Codegen-wise a move capture is indistinguishable from a by-value capture — both backends branch on `ast::is_reference_capture` rather than on `by_value` specifically, so `by_move` takes the existing value-capture path (a genuine copy at the bit level; only the move-checker enforces that the *source* is no longer usable). What remains unimplemented is the **implicit**-capture half of this chapter's rule: an escaping closure without an explicit `move [...]` list is not rejected or forced to move — implicit capture is unconditionally by value in every lowering, and no escape analysis exists yet to drive the default borrow-or-move choice.
