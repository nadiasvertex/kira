...
- Not yet enforced, left as follow-up: field/index-precise places (distinguishing `&mut p.a` from `&mut p.b`, currently rejected conservatively); the exclusivity of a *view* borrow (`&mut xs[a..b]`) against a plain borrow of the same collection, which — because a view may outlive the statement — needs the whole-function region tracking the [Views](15-views.md) chapter still lists as partial; and closure borrow-capture, which the [Closures and Capture](16-closures-and-capture.md) chapter covers. The source material's `E0013` example is illustrative only — this compiler attaches no numeric error codes to diagnostics.
