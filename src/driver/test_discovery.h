...
/// inline submodules literally named `tests` — at any nesting depth, since
/// an inline submodule is an ordinary module — and, unless a user source
...
/// suite. Each suite is named after the module that directly encloses its
/// `tests` submodule. See
...
/// parse error, or when a user source already declares `main`. Sources that
/// parse cleanly but declare no test at all are an error: `--test` was asked
/// to run tests and there are none, so say so here rather than let the
/// compile fail later with a missing-`main` error that never mentions
/// tests.
///
