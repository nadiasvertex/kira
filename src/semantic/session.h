...
struct submodule_declaration_record {
  ...
  bool is_inline = false; ///< Whether the declaration has an inline body (`module Name: ...`).
};
...
struct module_session_index {
  std::vector<module_file_record> module_files;                     ///< One entry per file with a `module` declaration.
  std::vector<submodule_declaration_record> submodule_declarations; ///< One entry per nested `module` item.
};
...
struct semantic_session {
  ...
  std::vector<semantic_symbol> symbols;                             ///< All symbols, indexed by `symbol_id`.
  std::vector<module_scope_record> module_scopes;                  ///< One record per distinct module scope.
  std::unordered_map<const ast::node *, scope_id> node_scopes;      ///< Scope active at each visited AST node.
};
