...
private:
  ...
  auto param_types_for(const ast::func_decl &decl, const module_members *owner)
    ...
    // One leaf per unannotated parameter, in the session store. The body's
    // own uses solve it — the same solver every other leaf has — and a
    // caller that meets it earlier ties its argument to it. Nothing here
    // walks the body.
    for (size_t i = 0; i < decl.params.size(); ++i) {
      const auto &param = decl.params[i];
      const auto name = param_name_of(param);
      if (param.type_annotation != nullptr || name.empty() ||
          (i == 0 && name == "self")) {
        continue;
      }
  ...
  /// its body. `pub` functions must annotate everything, and a generic
  /// function's parameters are its own type parameters' business.
  auto has_unannotated_params(const ast::func_decl &decl) -> bool {
    if (decl.visibility == ast::visibility::pub || !decl.type_params.empty()) {
      return false;
    }
    for (size_t i = 0; i < decl.params.size(); ++i) {
      const auto name = param_name_of(decl.params[i]);
      if (decl.params[i].type_annotation == nullptr && !name.empty() &&
          !(i == 0 && name == "self")) {
        return true;
      }
    }
  ...
  auto check_file(const ast::file &file) -> void {
    ...
    push_scope();
    // Functions with an unannotated parameter first: that parameter is a
    // leaf the body solves, and a caller checked before the body would meet
    // it open and, at any point needing a type, default it before the
    // body's own uses were read. Among themselves they need no order — a
    // call ties the caller's argument to the callee's leaf, and whichever
    // body is read later solves both.
    auto checked_early = std::unordered_set<const ast::node *>{};
    for (const auto &item : file.items) {
      if (item != nullptr && !item->has_error &&
          item->kind == ast::node_kind::func_decl &&
          has_unannotated_params(dynamic_cast<const ast::func_decl &>(*item))) {
        checked_early.insert(item.get());
        check_item(*item, /*at_module_scope=*/true);
      }
    }
    for (const auto &item : file.items) {
      if (item == nullptr || item->has_error ||
          checked_early.contains(item.get())) {
        continue;
      }
