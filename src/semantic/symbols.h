...
enum class symbol_namespace : uint8_t {
  ...
  associated_type_namespace,  ///< Associated types declared/defined in traits and impls.
};
...
enum class semantic_symbol_kind : uint8_t {
  ...
  pattern_binding_symbol,  ///< Name bound by a pattern (match arm, `if let`, `for`, ...).
  where_binding_symbol,    ///< Name bound by a trailing `where:` clause.
  type_parameter_symbol,   ///< Generic type/value parameter of a function, type, trait, impl, or concept.
  associated_type_symbol,  ///< Associated type declared in a trait or defined in an impl.
};
...
struct semantic_symbol {
  ...
  semantic_symbol_kind kind = semantic_symbol_kind::local_binding_symbol; ///< What kind of declaration this is.
  symbol_namespace name_space = symbol_namespace::value_namespace;       ///< Namespace the name is looked up in.
  ...
  scope_id defining_scope = k_invalid_scope_id;                          ///< Scope the symbol was added to.
};
